...
static void continue_sr_on_vad(struct speech_rec *sr)
{
    int errcode;
    const char *rslt;
    ...
    // we need to call QISRSessionBegin to continue listening
...
static void iat_callback(char *data, unsigned long len, void *user_para)
{
    int errcode;
                ...
                struct speech_rec_notifier * notify)
{
    ...
    if (aud_src == SR_MIC) {
        errcode = create_recorder(&sr->recorder, iat_callback, (void *) sr);
...
int sr_write_audio_data(struct speech_rec *sr, char *data, unsigned int len)
{
    ...
    if (MSP_REC_STATUS_SUCCESS == sr->rec_stat) { //�Ѿ��в�����д���
        printf("get result in MSP_REC_STATUS_SUCCESS\n");
        rslt = QISRGetResult(sr->session_id, &sr->rec_stat, 0, &ret);
        if (MSP_SUCCESS != ret) {
            sr_dbg("\nQISRGetResult failed! error code: %d\n", ret);
            end_sr_on_error(sr, ret);
